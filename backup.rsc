...
:do {
  ...
  :local T2U do={ # parses date formats: "hh:mm:ss","mm-dd hh:mm:ss","mmm/dd hh:mm:ss","mmm/dd/yyyy hh:mm:ss","yyyy-mm-dd hh:mm:ss"
  ...
  :local U2T do={
    ...
    :local mntStamp 12; :while (($prMntDays->$mntStamp)>$tmpDays) do={:set mntStamp ($mntStamp-1)}
    :local dayStamp [$ZeroFill (($tmpDays+1)-($prMntDays->$mntStamp))];
    :local timStamp (00:00:00+[:totime ($tmpSec%86400)]);
    :if ([:len $2]=0) do={:return "$yearStamp/$[$ZeroFill $mntStamp]/$[$ZeroFill $dayStamp] $timStamp"} else={:return "$timStamp"}
  }
  ...
  } else={:put "$[$U2T [$T2U]]\tDisk is specified by user is ACTIVATED"}
  :put "$[$U2T [$T2U]]\tWork is done on disk '$diskName'";
  ...
  :if ($maxDaysAgo>0) do={
    ...
    :local curTime [$U2T [$T2U]];
    :local curYear [:pick $curTime 0 4];
  ...
  :put "$[$U2T [$T2U]]\tEnd of saving settings and delete old files";
